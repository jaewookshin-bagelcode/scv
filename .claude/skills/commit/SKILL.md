...
**예시 1**
Input: scv-providers 에 OpenAI 채팅 컴플리션 스트리밍 어댑터 추가
Output: `feat(scv-providers): add OpenAI chat completions streaming adapter`

**예시 2**
Input: read 도구가 작업 폴더 밖 경로를 읽던 버그 수정
Output: `fix(scv-tools): reject read paths that escape the workspace root`

**예시 3**
Input: Provider 진입점을 stream 하나로 통합(비스트리밍 제거 — 호환성 깨짐)
Output:
```
refactor(scv-core)!: drop non-streaming complete(), keep stream() only

BREAKING CHANGE: 호출부는 complete() 대신 stream() 으로 이전해야 한다.
```
...
```sh
printf '%s' "<헤더>" | sh .claude/skills/commit/scripts/check-commit-header.sh -
```

원하면 `commit-msg` 훅으로 걸어 로컬에서 자동 검증할 수 있다:

```sh
ln -sf ../../.claude/skills/commit/scripts/check-commit-header.sh .git/hooks/commit-msg
...
> 실행 도구/환경의 규약을 따른다(모델·도구가 바뀔 수 있고, 이 스킬은 Codex 등으로도
> 포팅되므로 특정 도구에 묶지 않는다).
