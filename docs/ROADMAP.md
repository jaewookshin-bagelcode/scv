...
| `read` 도구 | ✅ 완료 |
| 권한 게이트 | ✅ 루프 fail-closed(`Allow` 만 실행) / ⚠ **대화형 승인 모달 미구현 → `Ask` 도구(`write`/`edit`/`bash`)는 승인 경로가 없어 호출 시 턴 거부**(요구사항·동작: ARCHITECTURE §4.3, 모달: 2a) |
| `SkillRegistry` 로더 | ✅ 완료 |
...
| `Provider::count_tokens` | ⛔ `Ok(0)` 스텁(→3a) |
...
| `ContextManager` 전략 | ✅ Clear/Noop 구현 / ⛔ Summarizing·루프 주입(→3b) |
...
- [ ] **2a. `scv-tui::App` 대화 루프 + 권한 모달 + 인터럽트 + 진행 표시.**
  ...
  - 권한 모달: **`Ask` 도구는 사용자의 명시적 승인을 받아야만 실행된다**(승인 전제 요구사항,
    ARCHITECTURE §4.3). fail-closed 라 모달이 동의를 받아 `Ask` → `Allow` 로 바꿔 돌려줘야
    실행되며, 이게 `write`/`edit`/`bash` 인터랙티브 사용의 **필수 선행 조건**이다. 이 모달이
    생기기 전까지는 `Ask` 도구 호출 시 턴이 `PermissionDenied` 로 거부된다.
...
- [ ] **3b. `ContextManager` 두 전략** — `ClearToolResultsManager`(tool_result 비우기) ✅
  구현 + 단위 테스트(`keep_recent` 최근 메시지 보존, 무손실 placeholder 치환).
  `SummarizingContextManager`(LLM 요약, Provider 호출 필요)와 임계 기반 **루프 주입**은
  미완(현재 `NoopContextManager` 주입). 요약은 Provider 호출이 필요해 별도 작업.
...
- [ ] **4e. 인증 타입 일반화.** (기본 프로바이더가 로컬 Ollama 로 바뀌면서 우선순위↑ — 현재
  `main.rs` 는 `api_key_env` 를 **필수**로 읽어 Ollama 도 `OLLAMA_API_KEY` 에 더미 값이 필요하다.
  `none` 지원이 들어오면 키 없이 진짜 out-of-box.) **결정(확정):** scv 는 LLM 을 *모델*로만 쓰고 도구·루프는
