...
>
> 남은 항목을 구현하면 **같은 PR 에서 이 문서의 해당 항목을 체크**한다
> (`AGENTS.md` § 단일 출처 규칙).
...
**Phase 0–4 가 구현 완료**됐다 — OpenAI·Anthropic·로컬 Ollama 로 한 턴이 end-to-end 로
흐르고(텍스트·도구 경로 모두, fake Provider 종단 테스트로 검증), 도구·권한·인터랙티브
TUI·세션 재개·컨텍스트 압축·세션 격리까지 동작한다. 남은 항목은 자체 하네스 원칙과
충돌해 **의도적으로 보류**한 선택 작업 `4f`(Codex 런타임) 하나다.

## 현재 상태 (Phase 0–4 구현 완료)
...
| 설정 로드(`scv-config`) | ✅ 다단계 병합(기본값→사용자→프로젝트→`SCV_*` env, figment) |
| `Provider::stream` (openai·anthropic) | ✅ openai SSE + HTTP 통합테스트 / ✅ anthropic SSE 디코더(단위테스트) |
| `to_wire` (요청 변환) | ✅ openai · ✅ anthropic(system 최상위·tool_use 객체·thinking) |
| `Provider::count_tokens` | ✅ openai 로컬 tiktoken / ✅ anthropic `/v1/messages/count_tokens` |
...
| `web_fetch`/`transcript_search` 도구 | ✅ 구현(web_fetch=Ask egress, transcript_search=Allow 세션 정밀검색) |
| 세션 격리(per-session worktree) | ✅ `--isolate` worktree(Drop 정리) / ⚠ 세션파일 동시쓰기 락은 잔여 |
...
- [x] **2a₀. (core 선행) 취소 + 통지 기반.** 완료 — ARCHITECTURE §2·§4.5·§6 대로.
  ...
  - ✅ `AgentEvent` enum(`scv-core::message`) + `Observer::on_event(&AgentEvent)` 로 확장.
    루프가 `Stream`/`ToolStart`/`ToolEnd { content, ... }`/`PermissionAsked`/`Interrupted` emit.
    `NullObserver`·`scv-tui::StreamObserver` 갱신(StreamObserver 는 토큰마다 stdout flush, 도구 출력 표시).
...
- [x] **3a. `Provider::count_tokens` 어댑터** — OpenAI 는 로컬 tiktoken(`o200k_base`)으로 추정
  ...
  구현**(4a 에서 구현 완료). compaction **트리거 주 신호는 `MessageStop` 의 usage**,
...
- [x] **4a. 두 번째 프로바이더(Anthropic) `stream`/`to_wire`** — 멀티 프로바이더 추상 실증.
  `to_wire`(system 최상위·tool_use 객체 input·tool_result content 블록·thinking adaptive +
  output_config.effort), SSE 디코더(`message_start`/`content_block_*`/`message_delta`/
  `message_stop` → 코어 StreamEvent, tool_use id 를 index 로 추적), `count_tokens`
  (`/v1/messages/count_tokens`). 순수 변환 + 상태 디코더는 단위 테스트, HTTP/SSE 부작용만
  `stream` 에. 코어/루프 변경 0 으로 붙어 추상이 새지 않음을 실증. `crates/scv-providers/src/anthropic.rs`.
- [x] **4b. `web_fetch` / `transcript_search` 도구** — `Tool` 구현 + 레지스트리 등록. 코어
  변경 0(`Tool` trait 만으로 추가). `web_fetch`(HTTP(S) GET, 본문 절단, 취소 관찰, **egress →
  `Ask`**, parallel_safe; default_registry 등록), `transcript_search`(세션 `<id>.jsonl` 부분문자열
  정밀 검색 → "session:lineno: text", **`Allow`**, parallel_safe; 세션 dir 주입이 필요해 scv-cli
  에서 등록). 단위 테스트 포함. `crates/scv-tools/src/{web_fetch,transcript_search}.rs`.
- [x] **4c. 세션 격리(per-session worktree)** — `--isolate` 시 cwd 가 git repo 면 세션별
  `git worktree`(같은 커밋의 독립 체크아웃)를 `~/.scv/worktrees/<id>` 에 만들어 그 경로를 도구
  workdir 로 준다. 종료 시 `Drop` 가 `git worktree remove --force` 로 정리. 비-git/실패면 cwd
  폴백(경고). `crates/scv-cli/src/workspace.rs`(임시 repo 로 worktree 생성·제거 단위 테스트).
  ⚠ **잔여**: 같은 세션 id 를 두 프로세스가 동시에 `--resume` 할 때의 세션 파일 append-only/락은
  미구현(저장소 통째 쓰기 — `session_store.rs` 주석에 한계 명시). 동시 *작업파일* 충돌은 worktree
  로 해소됨.
- [x] **4d. 설정 다단계 병합** — figment 으로 뒤가 앞을 덮는 레이어 병합 구현: 내장 기본값
  (serde default) → 사용자 `~/.scv/config.toml`(`SCV_CONFIG` override) → 프로젝트
  `./.scv/config.toml`(cwd 기준) → 환경변수 `SCV_*`(중첩 키는 `__`, 예 `SCV_AGENT__MAX_TOKENS`).
  누락 파일은 빈 레이어로 건너뛴다. CLI 플래그는 그 위에서 scv-cli 가 덮는다. 단위 테스트
  (프로젝트가 사용자 덮어쓰기 / 누락 프로젝트 건너뛰기). `crates/scv-config/src/lib.rs`.
...
- [ ] **4f. (TODO / 향후) Codex 런타임 — 구독/워크스페이스 권한 경로.** ChatGPT/Codex 구독·
  ...
  item/agentMessage/delta·turn/completed). `dynamicTools`/`item/tool/call` 은 **공식
  experimental API**(`capabilities.experimentalApi = true` 필요)다 — 비공식이 아니라 실험적.
  **트레이드오프(확정): 루프·도구·승인을 Codex 가 소유** → scv 의 `run_turn`/`ToolRegistry`/
  `PermissionGate` 미사용(="하네스 직접 구축/도구는 scv 만" 원칙과 충돌). 도구는
  `--sandbox read-only --ask-for-approval never -c web_search=disabled` 로 *줄일* 뿐 0 으로 끄는
  스위치는 없음. 실험적·무거움 → **구독이 자체 하네스보다 우선일 때만** 착수.
