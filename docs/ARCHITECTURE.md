...
flowchart BT
    ...
    O["OpenAiProvider<br/>로컬 Ollama(localhost:11434) · OpenAI 클라우드<br/>POST /chat/completions (SSE)"]
...
`anthropic-beta` 헤더가 있을 때 200, 없을 때 400 으로 갈려 프록시 통과와 context editing
수용을 확인했다. 서버 요약형 compaction 은 직접 트리거하지 않아 미검증이다. 그럼에도 비교 후
**로컬을 기본으로** 둔다:
...
| 캐시 상호작용 | 현재 scv 는 tools+system 에만 캐시 마커를 둠 | messages 편집은 tools+system 캐시를 깨지 않음 |

**결론(측정 → 기준 적용)**: 투명·소유 철학 + 무손실 재조회 + 프로바이더 독립성 때문에
컨텍스트 관리는 **로컬**이 맞다. 얻는 것은 통제·무손실, 포기하는 것은 Anthropic 이 가져갈 수
있었던 구현 부담 절감. 매우 긴 Anthropic 전용 대화에서 구현 부담이 더 크면 서버 context editing
을 옵션으로 둘 여지는 있다(미구현).
