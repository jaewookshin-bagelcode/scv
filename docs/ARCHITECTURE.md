...
**서버사이드 도구와 `pause_turn`(ROADMAP 5c).** 루프는 `stop_reason` 을 세 갈래로 다룬다:
`tool_use` → **로컬** 도구 실행 후 `tool_result` 를 user 메시지로 push(위 그림). `pause_turn`
→ 서버사이드 도구(web_search 등)가 실행 도중 일시정지한 것이므로 **로컬 실행도 user 메시지
추가도 하지 않고**, 직전 assistant(서버 `tool_use` 블록 포함)까지의 히스토리를 **그대로 다시
보내** 재개한다(`max_tool_iterations` 가 무한 pause 방지). 그 외(`end_turn` 등) → 턴 종료.
즉 "내가 실행하는 로컬 도구"와 "서버가 실행하고 결과만 통과시키는 도구"를 한 루프가 함께
다룬다 — 서버사이드 web 도구(5d·5e)의 구조적 전제.
...
context editing(`clear_tool_uses_20250919`)·compaction(`compact_20260112`)을 제공한다.
측정(aiproxy Sonnet): **둘 다 프록시로 동작 확인** — `anthropic-beta` 헤더가 프록시를 그대로
통과한다(가짜 베타 값은 Anthropic 이 거부하는 것으로 passthrough 입증). 그럼에도 비교 후 **로컬을
기본으로** 둔다:

| 축 | 로컬(scv `Clear`/`Summarizing`) | 서버(Anthropic, 프록시로 동작) |
|---|---|---|
| 가용성 | 모든 프로바이더(ollama·openai 포함) | Anthropic 만 |
| 제어/투명성 | 언제·무엇 자를지 명시(`threshold`·`keep_recent`) | 서버 정책 |
| 무손실 재조회 | `Clear`=무손실(원본 디스크/JSONL+`read`/`grep`) | 제한적 |
| 비용 | `Clear`=LLM 0, `Summarize`=1 | 서버 처리 |
| **캐시 상호작용** | scv 가 prefix 안정성 통제 | **편집·요약이 prefix 를 재작성 → §4.1 캐시 무효화** |

**결론(측정 → 기준 적용)**: 투명·소유 철학 + 무손실 재조회 + **캐시 prefix 통제**(서버 편집은
5b 캐시를 깬다) + 프로바이더 독립성 때문에 컨텍스트 관리는 **로컬**이 맞다. 얻는 것은 통제·무손실·
캐시 보존, 포기하는 것은 Anthropic 이 가져갈 수 있었던 구현 부담 절감. 매우 긴 Anthropic 전용
대화에서 구현 부담이 더 크면 서버 compaction 을 옵션으로 둘 여지는 있다(미구현).
...
**서버 vs 로컬 도구 배치(web_search·web_fetch) — 측정 근거로 결정(ROADMAP 5d·5e).**
같은 "웹 접근"이라도 둘을 다르게 둔다. 측정(aiproxy Sonnet, 직접 호출):

| 축 | 로컬 `web_fetch`(scv `Tool`) | 서버 web_fetch / web_search(Anthropic) |
|---|---|---|
| 실행·왕복 | scv 가 실행, tool_use→tool_result 왕복 1+ | 서버 실행, 한 응답 내 완결(`server_tool_use`→`*_tool_result`→text, 왕복 0). 둘 다 프록시로 동작 확인 |
| 권한 게이트 | `Ask` 모달로 egress 통제 가능 | scv 가 가로채지 못함(서버 실행) |
| 네트워크/자원 | scv egress — 사내망·localhost·인증 URL 접근 | Anthropic egress — 공개만, 사내망 불가 |
| URL 출처 | 모델이 준 임의 URL | (web_fetch) 대화에 이미 있는 URL만 |
| 감사·가공 | 가져온 내용 tool_result 로 기록, 절단 직접 제어 | 서버가 fetch·요약, 클라이언트 통제 약함 |
| 외부 인덱스 | 불가(로컬은 검색 인덱스가 없음) | web_search 가능 |

**판단 기준**: ① 부작용/egress 통제·사내망/로컬 자원 접근·감사가 핵심이면 **로컬**, ② 로컬이
재현 못 하는 외부 인덱스(검색)이거나 왕복 제거 이득이 크면 **서버**. 이 기준으로 scv 는
**web_search 는 서버**(외부 인덱스라 로컬 재현 불가 — 5d 구현), **web_fetch 는 로컬 기본**
(권한·사내망·감사 통제가 scv 의 투명·소유 철학과 직결)으로 둔다. 서버 web_fetch 도 프록시로
동작하므로, 통제가 불필요한 공개 URL 대량 처리 등에는 web_search 처럼 opt-in 토글로 추가할 수
있다(미구현). 즉 결론은 "측정 → 기준 적용"으로 도출된 것이지 선험적 가정이 아니다.
...
기능들이 Anthropic 전용이라 변수 통제가 필요하기 때문이다. caching 은 본질상 서버 기능,
web_search 는 서버사이드 전환(과제 지시)이지만, **web_fetch·compaction 의 서버 vs 로컬은
측정·비교로 판단**한다 — 결론을 미리 박지 않고 토큰·비용·권한·감사·캐시 prefix 상호작용을
실측해 근거와 함께 도출하는 것이 과제의 핵심이다. 상세 순서는 [`ROADMAP.md`](./ROADMAP.md) Phase 5.
