...
기본 프로바이더는 **aiproxy 경유 Anthropic(`claude-sonnet-4-6`)** 이고, 로컬
Ollama(`qwen3.5:9b`)는 오프라인 대체다. 아래
...
- **모델 ID 는 설정에서 주입**, 코드에 하드코딩하지 않는다(어댑터 기본값 1개만 허용).
  기본 모델 `claude-sonnet-4-6`(aiproxy Anthropic). 날짜/임의 접미사를 붙이지 않는다.
...
### OpenAI / OpenAI-호환 어댑터

로컬 Ollama 가 이 어댑터를 재사용한다. `kind` 로 세 변형:
