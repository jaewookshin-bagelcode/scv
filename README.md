...
| `/skills` | 사용 가능한 스킬 목록 |
| `/<skill>` | 그 스킬을 발동(예: `/compact` → 대화 요약). 본문 절차를 주입해 그 턴에 적용 |
| `/help` | 명령 도움말 |
| **PageUp / PageDown** | 대화 로그 스크롤(이전 대화 보기 / 하단 복귀) |

예: `/provider openai` → openai 설정 모델로 전환, `/model gpt-5.4-mini` → 모델만 교체,
`/compact` → 대화를 요약. 전환은 현재 세션을 유지한 채 다음 턴부터 적용된다. (클라우드
프로바이더는 해당 키 환경변수가 있어야 함.)
...
- **발동(사용)**: TUI 에서 **`/<스킬이름>`** 으로 호출한다(예: `/compact`). 그 스킬의 본문
  절차를 그 턴에 주입해 모델이 따르게 한다. `/skills` 로 목록 확인.
